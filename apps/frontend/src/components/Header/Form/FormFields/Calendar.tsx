...
const Calendar = () => {
  ...
  const minimize = useSelector((store: any) => store.app.minimize);
  ...
  useEffect(() => {
    function findMonthWidth() {
      let width = onHouseDetailPage && !minimize ? 375 : window.innerWidth <= 956 ? 384 : 440; // Width of each month component
      ...
      for (let i = 0; i < 7; i++) {
        ...
        let onClickHandler: (() => void) | null = () => {
          if (isModalOpen && onCheckOutPage) {
            if (!isPastDate) onCheckoutDateClick(cloneDay);
          } else if (isModalOpen) {
        ...
        days.push(
          <div
            key={day.toString()}
            className={`  relative ${onCheckOutPage ? '!h-[2.62rem]   !w-[2.62rem]' : ''} ${
              onHouseDetailPage && !minimize
          ...
          >
            <div
              className={`${onCheckOutPage ? '!h-[2.62rem]   !w-[2.62rem]' : ''} ${
                onHouseDetailPage && !minimize
  ...
  const onCheckoutDateClick = (day: Date) => {
  ...
  const handleScroll = (direction: string) => {
    const maxIndex = 2; // 3 months rendered, so max index is 2
  ...
  useEffect(() => {
    function helperFunction() {
      let preventDefault = (e: any) => e.preventDefault();
      const container = scrollContainerRef.current;
      if (window.innerWidth >= 744) {
        if ((!onHouseDetailPage && !minimize) || onCheckOutPage) {
          (container as any)?.addEventListener('wheel', preventDefault, {
            passive: false,
          });
          (container as any)?.addEventListener('touchmove', preventDefault, {
            passive: false,
          });
        }
      } else {
        // Remove event listeners for smaller screens to allow scrolling
        (container as any)?.removeEventListener('wheel', preventDefault);
        (container as any)?.removeEventListener('touchmove', preventDefault);
      }
      // ...
    }
    helperFunction();
  ...
  return (
    <div
      className={`flex w-full ${!onCheckOutPage && '1xz:w-96'} 1md:w-full min-h-[400px]

       flex-col justify-center  relative`}
    >
      <div
        className={` ${onHomePage || onHouseDetailPage ? 'absolute' : ''}  top-[3.6rem] ${
          onCheckOutPage &&
      ...
      <div
        className={`absolute   ${onCheckOutPage && '!right-[0.8rem] !translate-x-0'}  ${
          onHouseDetailPage && !minimize
      ...
      <button
        ...
        className={` ${onCheckOutPage ? 'hidden 1xz:block !left-8' : ''} absolute ${
          currentIndex === 0 ? 'opacity-30 cursor-not-allowed' : 'hover:bg-gray-100'
        } ${
          onHouseDetailPage && !minimize ? 'left-0' : '1md:left-8 left-0 hidden 1xz:block'
        } top-[1.2rem] transform -translate-y-1/2 z-0 bg-white p-2 rounded-full  `}
      ...
      <button
        ...
        } ${onHouseDetailPage && !minimize ? 'right-0' : '1md:right-8 right-0 hidden 1xz:block'} ${
      ...
      <div
        ...
        className={` h-full overflow-x-hidden  ${onCheckOutPage && '!w-full'}  ${
          onHouseDetailPage && !minimize
      ...
      >
        <div
          ...
          className={`inline-flex ${onCheckOutPage ? '!gap-x-8' : ''}  w-full ${
            onHouseDetailPage ? '  1xlx:w-full  1xlx:overflow-visible ' : 'w-full'
          }  h-[400px]  1xz:h-auto ${
            onHouseDetailPage && !minimize
          ...
          {Array.from({ length: 3 }, (_, index) => (
            <div
              ...
              className={`   justify-center w-80 ${
                onCheckOutPage ? `${index <= 0 ? '1xz:!pl-4' : '!pl-0'} !mx-1 ` : ''
              }  items-center ${
                onHouseDetailPage && !minimize
                  ? `w-full flex !mx-1  flex-col
                   h-full  ${index <= 0 ? '1xlx:pl-0 1xs:pl-3 pl-1 ' : '1xlx:pl-[4.6rem] '}`
                  : ` w-full flex  flex-col  ${
                      index <= 0 ? '1md:pl-8 ' : '1md:pl-16 '
                    } justify-between h-full ${
                      onHouseDetailPage && minimize ? '1xz:gap-y-0' : '1xz:gap-y-10'
                    } gap-y-6  `
              } 1xz:mx-6 1md:mx-1 w-full rounded-lg`}
            >
              <div
                className={`flex w-full ${
                  onHouseDetailPage ? 'justify-center' : '1xz:justify-center justify-start'
                } `}
              >
                {renderHeader(addMonths(currentMonth, index))}
              </div>

              <div className={`w-full ${onHouseDetailPage ? '1xz:mt-10 mt-10' : ''} 1md:w-auto`}>
