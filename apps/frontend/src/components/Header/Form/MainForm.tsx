...
import { setActiveInput, setMinimizeFormBtn, setOpenName } from '../../../redux/mainFormSlice';
...
const useMinimizeButtonEffect = (minimizeFormBtn, minimize, dateOption, dispatch) => {
  useEffect(() => {
    if (!minimize) return;
...
const getFormClasses = (minimize, startScroll, curSelectInput) => {
  ...
  return `transition-all ${!minimize ? (!startScroll ? 'animate-formBlur' : '') : ''} ${
  ...
  useEffect(() => {
    function handleClick(e) {
      if (isCalendarModalOpen) {
        return;
      } else if (!openName && headerRef?.current && !headerRef.current?.contains(e.target)) {
        setMinimize(false);
      }
    }
...
const BackGroundModal = ({
  ...
  return ReactDOM.createPortal(
    <>
      <div
        className={`${onHouseDetailPage ? 'absolute' : 'fixed top-0'} opacity-40 w-full h-${
          minimize ? 'full' : '0'
        } bg-black`}
      ></div>
    </>,
...
const SearchButton = ({ label, content, onClick, extraClasses = '', width = 'w-full' }) => {
  return (
    <button
      onClick={onClick}
      className={`text-[1.8rem] h-[6rem] flex-center px-4 font-normal ${extraClasses}`}
    >
      {label === 'Anywhere' && <img src={houseIcon} alt="House" className="w-24 h-24 mr-3" />}
      <span className={`${width} text-ellipsis overflow-hidden whitespace-nowrap`}>
  ...
  return (
    <button className="text-3xl flex-center pr-3 h-[6rem]">
      <p
        onClick={onClick}
        className={`${
          content ? 'text-black font-normal' : 'text-black font-normal'
...
const SearchForm = ({ minimize }) => {
  ...
  return (
    ...
    >
      ...
      />
      <Divider />
      <GuestButton content={displayGuestInput} onClick={() => handleMinimize('guest')} />
    </div>
  );
};
...
const MainForm = ({ headerRef }) => {
  const { curSelectInput, minimizeFormBtn, dateOption, openName, isCalendarModalOpen } =
  ...
  const startScroll = useSelector((store: any) => (store as any)[sliceName]?.startScroll);
    ...
    <div className="flex items-center   flex-col">
      <div className={formClass}>
        {!startScroll && !minimize ? (
          <MainComponent startScroll={startScroll} minimize={minimize}></MainComponent>
        ) : (
