...
// Day 1: interactive sub-components removed; keep placeholders only
import {
  setCurrentDot,
  setMonths,
  setOpenWhenCard,
...
const useDateManagement = () => {
  const startDate = useSelector((state: any) => state.form.selectedStartDate);
  const endDate = useSelector((state: any) => state.form.selectedEndDate);
  const currentDot = useSelector((store: any) => store.form.curDot);
  const startDurationDate = useSelector((store: any) => store.form.startDurationDate);
  const textForFlexibleInput = useSelector((store: any) => store.form.textForFlexibleInput);
  ...
  const showReset =
    ...
    currentDot !== 3 ||
    format(startDurationDate, 'yyyy-MM-dd') !== format(new Date(), 'yyyy-MM-dd');
...
const FooterButtons = ({ showReset, onReset, onNext }) => (
  <div className="w-full py-3">
    <div className="flex px-5 justify-between items-center">
      ...
      <button onClick={onNext} className="px-12 py-3 rounded-lg bg-black text-white">
        Next
      </button>
    </div>
  </div>
  ...
  return (
    <div className="w-full relative overflow-hidden pt-5 h-[calc(100vh-10rem)] flex flex-col shadow-lg border border-shadow-gray bg-white rounded-2xl">
      <div className="flex flex-col w-full h-full">
        ...
        >
          <FooterButtons showReset={showReset} onReset={handleReset} onNext={handleNext} />
        </div>
      </div>
    </div>
  );
