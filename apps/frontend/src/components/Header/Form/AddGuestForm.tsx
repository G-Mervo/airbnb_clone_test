...
import { handleSearch, useHandleCrossClick } from '../../../hooks/MainFormContent';
...
const useGuestForm = () => {
  ...
  const handleSearchClick = () => {
    ...
    handleSearch({
      ...
      textForFlexibleInput,
      textForGuestInput,
      dateFlexibility: formState.dateFlexibility,
    });
...
const SearchButton = ({ curSelectInput, onClick }) => (
  <div
    ...
    className={`hover:bg-dark-pink 1xz:mr-2  ${
      curSelectInput ? '1smd:w-[8rem] 1xz:w-[3rem]  z-50' : 'w-[3rem] 1smd:mr-0 z-50 '
    } hover:cursor-pointer flex items-center ${
      curSelectInput ? '1xz:justify-center 1smd:justify-start' : 'justify-center'
    } duration-200 ease-out ${
      curSelectInput ? 'bg-dark-pink ml-[-1.6rem] mr-2' : 'bg-pink ml-[-0.5rem]'
    } rounded-full h-[3rem]`}
  >
    <img className={curSelectInput ? '1smd:pl-2 1smd:pr-1' : ''} src={searchIcon} alt="Search" />
    {curSelectInput && <p className="text-center 1xz:hidden 1smd:block text-white">Search</p>}
  </div>
);
    ...
    <Modal onlyOneTime={onlyOneTime}>
      <div
        ...
        className={`flex 1xz:relative 1smd:static 1smd:w-[17.7rem] ${
          curSelectInput === 'addGuest' ? 'rounded-full bg-white shadow-AddGuestShadow' : ''
      ...
      >
        <Modal.Open opens="addGuest">
          <div className="flex justify-center items-center">
            <div
              ...
              ${curSelectInput === 'addGuest' ? '' : 'before:hover:bg-grey-light-50'}
        ...
        <SearchButton curSelectInput={curSelectInput} onClick={handleSearchClick} />
      </div>

      <Modal.Window resetRef={addGuestResetRef} modalRef={modalRef} name="addGuest">
        <AddGuest />
      </Modal.Window>
    </Modal>
  );
