...
// Day 1: keep UI, but no dynamic destination asset import
const Destination: { iconName: string; link: string }[] = [];
import {
...
const SearchInput = ({ defaultValue, onInputChange, showNextButton, onNextClick }) => (
  <label
  ...
  >
    ...
    />
    {showNextButton && (
      <button onClick={onNextClick} className="bg-black text-sm text-white px-4 py-2 rounded-full">
        Next
      </button>
    )}
  </label>
...
const MobileWhereCard = () => {
  ...
  const { region, destinationInputVal } = useSelector((store: any) => store.form);
