...
const formatMonthsList = (months: { month: string }[]) => {
...
const useFlexibleDates = () => {
  ...
  const { months: selectedMonths, stayDuration } = useSelector((store: any) => store.form);
...
const FlexibleContent = ({
  ...
  selectedMonthsInfo,
  stayDuration,
}: { currentInput: string; selectedMonthsInfo: { month: string }[]; stayDuration: string }) => {
...
const FlexibleInputWrapper = ({
  ...
  onMouseEnter,
  onMouseLeave,
}: { currentInput: string; children: React.ReactNode; onClick: (e: React.MouseEvent) => void; onMouseEnter?: () => void; onMouseLeave?: () => void }) => {
  return (
    <div
...
const Flexible = ({ modalRef, handleInputField, flexibleRef, onlyOneTime }: { modalRef: React.RefObject<HTMLDivElement>; handleInputField: (target: EventTarget | null, name: string) => void; flexibleRef?: React.RefObject<HTMLDivElement>; onlyOneTime?: React.MutableRefObject<boolean> | null }) => {
  const { curSelectInput: currentInput } = useSelector((store: any) => store.form);
