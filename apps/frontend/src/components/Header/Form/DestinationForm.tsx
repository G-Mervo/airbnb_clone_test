...
const useDestinationForm = () => {
  ...
  const { curSelectInput, destinationInputVal, region } = useSelector((store: any) => store.form);
  ...
  console.log('DestinationForm - curSelectInput:', curSelectInput);
  console.log('DestinationForm - destinationInputVal:', destinationInputVal);
  console.log('DestinationForm - region:', region);
...
const DestinationInput = React.forwardRef(({ value, onChange, isActive, placeholder }, ref) => (
  <input
    ref={ref}
    onChange={(e) => onChange(e.target.value)}
    type="text"
    className={`1smd:w-[10.62rem] text-sm font-medium
      outline-none focus:outline-none placeholder:text-sm bg-transparent
      placeholder:font-extralight placeholder:text-gray-500`}
    id="destination"
...
const DestinationForm = ({ onlyOneTime, buttonRef, inputRef, modalRef }) => {
  ...
  const showClearButton =
    (region !== 'all' || destinationInputVal) && curSelectInput === 'destination';
        ...
        <Modal.Open opens="destination">
          <div
            ...
            className={`flex 1xz:relative 1smd:static
              ${curSelectInput === 'destination' ? 'shadow-destinationShadow rounded-full' : ''}
          ...
          >
            <label
              ...
              className={`1smd:w-[17.67rem] hover:before:content-['']
                ...
                ${curSelectInput === 'destination' ? 'rounded-full w-full bg-white' : ''}
                ${curSelectInput === 'destination' ? '' : 'before:hover:bg-grey-light-50'}
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <DestinationInput
                    ...
                    isActive={!curSelectInput || curSelectInput === 'destination'}
                    placeholder="Search Destinations"
                  />
                </div>
                {showClearButton && (
                  <ClearButton onClick={(e) => handleCrossClick(e, 'destination')} />
                )}
              </div>
            </label>
          </div>
        </Modal.Open>
        <Modal.Window modalRef={modalRef} name="destination" className="translate-y-[50px]">
          <Destination />
        </Modal.Window>
      </Modal>
    </div>
